...
function requireMasterUser(req, res, next) {
    ...
    if (!db.isMasterUser(userId)) return res.status(403).json({ error: 'Only master users can perform this action' });
    next();
}
...
app.post("/user/create", (req, res) => {
    ...
    const id = uuid.v4();
    db.createUser({ id, username, role: role || 'normal' });
    // Return the actual DB record — ON CONFLICT keeps the original id, so we must look it up
    const actual = db.getUserByUsername(username);
...
app.post("/agent/new/", requireMasterUser, async (req, res) => {
    ...
    const allFinalized = Object.values(db.getAllFinalizedAgents()).flat();
...
app.post("/agent/finalize", requireMasterUser, (req, res) => {
    ...
    db.saveAgentGroup({ id });
    db.saveAgents(id, records);
...
app.get("/agents/finalized", (req, res) => {
    res.json({ agents: db.getAllFinalizedAgents() });
});
...
app.post("/chat/orchestrate", async (req, res) => {
    ...
    // Detect master user
    const userId = req.headers['x-user-id'] || req.body?.userId;
    const isMasterUser = userId ? db.isMasterUser(userId) : false;
    ...
    try {
        // ── 1. RAG context ────────────────────────────────────────────────────
        let ragContext = '';
        const collectionName = groupId ? db.getQdrantCollection(groupId) : null;
        if (collectionName) {
            try {
        ...
        const companyName = (groupId ? db.getCompanyName(groupId) : null) || '';
        ...
        if (pendingContext?.orchestration) {
            ...
            const agentList = agentsWithMeta.map((a, i) => `${i + 1}. Name: "${a.name}" | Role: "${a.role}"`).join('\n');
...
3. Pick the single best agent to handle this.
...
Return ONLY valid JSON:
{
  ...
  "reformatted_query": "<user request rewritten clearly, in user's exact script/style>"
        ...
        if (chosenName === META_AGENT_NAME || chosenName.toLowerCase().includes('meta')) {
            ...
            db.saveChatMessage({ id: uuid.v4(), groupId, userMessage: message, response, agentName: META_AGENT_NAME, agentRole: 'Database Manager' });
...
4. Only set "ready": false if you GENUINELY need specific user-provided data (like their name, email, project details) that is NOT available anywhere above.
...
Return ONLY valid JSON:
{
  ...
  "action_data": {}
                ...
                db.saveChatMessage({ id: uuid.v4(), groupId, userMessage: message, response: finalResult.response, agentName: agent.name, agentRole: agent.role });
                return res.json({
                    status: 'complete',
...
MUST reply in ${userLanguage}. Return ONLY valid JSON: {"response":"<answer>","intent_understood":"<intent>","action":null,"action_data":{}}`;
        ...
        db.saveChatMessage({ id: uuid.v4(), groupId, userMessage: message, response: forceResult.response, agentName: agent.name, agentRole: agent.role });
...
async function main(url) {
    ...
    const validPoints = points.filter((point, index) => {
        ...
        db.saveAgentGroup({ id: agentId, sourceUrl: url, qdrantCollection: collectionName, industry: industryInfo.industry, businessType: industryInfo.business_type, companyName: industryInfo.company_name });
        db.saveAgents(agentId, agents);
...
app.get("/chat/history", (req, res) => {
    ...
    res.json({ history: db.getChatHistory(groupId, limit ? parseInt(limit) : 50) });
});
